...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    vec = [wordVectors[tokens[i]].reshape(1,-1) for i in sentence]
    vec_matrix = np.vstack(vec)
    #print vec_matrix.shape
    sentVector = np.sum(vec_matrix,axis=0)
...
def getRegularizationValues():
    ...
    ### YOUR CODE HERE
    values = [10**i for i in range(-3,3)]
    ### END YOUR CODE
...
def chooseBestModel(results):
    ...
    max_acc=max([ i['test'] for i in results])
    bestResults=[ i for i in results if i['test']==max_acc]
