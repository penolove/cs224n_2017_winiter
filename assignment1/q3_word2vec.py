...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    rows_norms = np.linalg.norm(x, axis = 1).reshape(-1, 1)
    x /= rows_norms
    ### END YOUR CODE
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    ### YOUR CODE HERE
    #KxD
    U_new = outputVectors[indices] 
    V_c = predicted.reshape(-1,1)
    #Kx1
    sign = - np.ones(len(indices)).reshape(-1, 1)
    sign[0] = 1
    product = sigmoid(U_new.dot(V_c)* sign)
    cost = - np.log(product).sum()
    
    #Kx1
    product_1 = (product - 1) * sign
    gradPred = U_new.T.dot(product_1.reshape(-1,1))
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    ### YOUR CODE HERE
    curr_idx = tokens[currentWord]
    # v should be Dx1
    v_hat = inputVectors[curr_idx].reshape(-1,1)
    target_indices = [tokens[word] for word in contextWords]
    for o in target_indices:
       cost_ ,gradv ,gradU = word2vecCostAndGradient(v_hat, o, outputVectors, dataset)
       cost = cost + cost_
       gradOut = gradOut+gradU
       gradIn[curr_idx] = gradIn[curr_idx] + gradv.reshape(-1)
    ### END YOUR CODE
