...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        extract = lambda target_window : [ select(i) for i in target_window]

        count=0
        while count<sen_end: 
            target_window = range (count-window_size,count+window_size+1)
            windowed_data.append( (flatten(extract(target_window)),labels[count]) )
            count+=1
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        # place_holder: N x n_window_features -> features N x n_window_features x embed_size
        features = tf.nn.embedding_lookup(embedding, self.input_placeholder)
        # embeddings: Nx (n_window_features x embed_size)
    ...
    def add_prediction_op(self):
        ...
        """
        # x with N x (n_window_features X embed_size)
        x = self.add_embedding()
        dropout_rate = self.dropout_placeholder
        ### YOUR CODE HERE (~10-20 lines)
        embed_size = self.config.embed_size
        hd_size  = self.config.hidden_size
        nwind_size = self.config.n_window_features
        cls_size = self.config.n_classes
        W = tf.get_variable("W", shape=(embed_size*nwind_size, hd_size), initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable("b1", shape=(hd_size,), initializer=tf.contrib.layers.xavier_initializer())
        U = tf.get_variable("U", shape=(hd_size, cls_size), initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable("b2", shape=(cls_size,), initializer=tf.contrib.layers.xavier_initializer())
